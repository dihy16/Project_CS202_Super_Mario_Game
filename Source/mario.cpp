...
//big note: everytime u need to recall size via marioArea, multiply it by 1.5
//coordinates in this file is taken directly via coordinates of marioSprite. Feel free to declare global coordinates if u need
...
void Mario::moveRight(IntRect &rect, RenderWindow &window)
{
   ...
   if (window.getSize().x - marioSprite.getPosition().x - 1.5 *  marioArea.width / 2 < 0)
...
void Mario::moveLeft(IntRect &rect, RenderWindow &window)
{
   ...
   if (marioSprite.getPosition().x - 1.5 * marioArea.width / 2 < 0)
